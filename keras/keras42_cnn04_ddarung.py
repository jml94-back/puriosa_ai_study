...
# x,y 분리
x = train_csv.drop(['count'],axis = 1) # axis 0 행 1 열 (1459, 9)
y = train_csv['count']
...
x_train = np.array(x_train).reshape(-1,3,3,1)
x_test = np.array(x_test).reshape(-1,3,3,1)

#2. 모델
model = Sequential([keras.Input(shape=x_train[0].shape)])
model.add(Conv2D(32, (2,1),input_shape=x_test[0].shape))
