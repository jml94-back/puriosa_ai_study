...
for x1 in x_train:
    x2 = [np.roll(x1, i) for i in range(len(x1))]
    x_train2.append(x2)
...
# exit()
x_train = np.array(x_train2).reshape(-1,8,8,1)
x_test = np.array(x_test2).reshape(-1,8,8,1)

#2. 모델
model = Sequential()
model.add(Conv2D(32, (4,4),input_shape=x_test[0].shape)) # (26,26,64)
